...
def zip_with_children_from_root(
    tree: TreeLike[A],
) -> TreeLike[Tuple[A, dict[BinTreeDirection, A]]]:
...
def subtree_rooted_at_node_by_path(
    tree: TreeLike[A], path: Tuple[BinTreeDirection, ...]
) -> TreeLike[A]:
    subtree: TreeLike[A] = {}
...
def replace_subtree_with_tree(
    tree: TreeLike[A],
    target_path: Tuple[BinTreeDirection, ...],
    new_sub_tree: TreeLike[A],
) -> TreeLike[A]:
...
class FoldExpressionTree_3625(Scene):
    ...
    def construct(self):
        ...
        left_plus = ExprNode(
            (BinTreeDirection.LEFT,), MathTex("+", **NODE_CONFIG).move_to(LEFT * 2)
        )
        right_times = ExprNode(
            (BinTreeDirection.RIGHT,),
            MathTex("\\times", **NODE_CONFIG).move_to(RIGHT * 2),
        )
        leaf3 = ExprNode(
            (BinTreeDirection.LEFT, BinTreeDirection.LEFT),
            MathTex("3", **NODE_CONFIG).move_to(LEFT * 3 + DOWN * 2),
        )
        leaf6 = ExprNode(
            (BinTreeDirection.LEFT, BinTreeDirection.RIGHT),
            MathTex("6", **NODE_CONFIG).move_to(LEFT * 1 + DOWN * 2),
        )
        leaf2 = ExprNode(
            (BinTreeDirection.RIGHT, BinTreeDirection.LEFT),
            MathTex("2", **NODE_CONFIG).move_to(RIGHT * 1 + DOWN * 2),
        )
        leaf5 = ExprNode(
            (BinTreeDirection.RIGHT, BinTreeDirection.RIGHT),
            MathTex("5", **NODE_CONFIG).move_to(RIGHT * 3 + DOWN * 2),
        )

        tree: TreeLike[MathTex] = dict(
            [
                (n.path, n.node_value)
                for n in [root, left_plus, right_times, leaf3, leaf6, leaf2, leaf5]
            ]
        )
        ...
        edge_from_parent: TreeLike[Line | None] = dict(
            [
                (
                    path_to_parent + (direction,),
                    connect(parent, child, BUFF, LINE_CONFIG),
                )
                for path_to_parent, (parent, children) in zip_with_children_from_root(
                    tree
                ).items()
        ...
        def animate_reduction(
            ...
            def vgroup_containing_direct_children_along_with_node_at(
                path: Tuple[BinTreeDirection, ...],
            ) -> VGroup:
                nodes = [
                    n
            ...
            modified_body_double = current_expr_body_double.scale(0.8).set_x(
                current_expr.get_center()[0]
            )
            self.remove(current_expr)
            self.add(modified_body_double)  # *hard-swap*

            subtree = vgroup_containing_direct_children_along_with_node_at(
                path_to_redex
            )
            subtree_box = SurroundingRectangle(subtree, color=ORANGE, buff=0.3)
            subtree_expr_box = SurroundingRectangle(
                modified_body_double.get_part_by_tex(redex_part_in_expr),
                color=ORANGE,
                buff=0.1,
            )
            ...
            new_value_node = MathTex(new_value, **NODE_CONFIG).move_to(
                current_expr_tree[path_to_redex].get_center()
            )
            new_value_box = SurroundingRectangle(new_value_node, color=ORANGE, buff=0.1)

            new_expr = (
                MathTex(*new_expr_parts, arg_separator="", **NODE_CONFIG)
                .scale(0.8)
                .set_x(current_expr.get_center()[0])
            )
            new_expr_box = SurroundingRectangle(
                new_expr.get_part_by_tex(new_value), color=ORANGE, buff=0.1
            )
            ...
            self.play(
                FadeOut(new_value_box),
                FadeOut(new_expr_box),
                run_time=SLEEP_BETWEEN_REDUCTION_STEPS / 2,
            )
            self.wait(SLEEP_BETWEEN_REDUCTION_STEPS / 2)

            return (
                replace_subtree_with_tree(
                    current_expr_tree, path_to_redex, {(): new_value_node}
                ),
                new_expr,
            )
        ...
        edges = VGroup(
            *[edge for edge in edge_from_parent.values() if edge is not None]
        )
        expr = (
            MathTex("(3+6)+(2\\times5)", arg_separator="", **NODE_CONFIG)
            .scale(0.8)
...
class SearchRedexAndFoldExpressionTree_13479(Scene):
    ...
    def construct(self):
        ...
        left_plus = ExprNode(
            (BinTreeDirection.LEFT,),
            MathTex("+", **NODE_CONFIG).move_to(LEFT * 1.25 + UP * 2),
        )
        left_left_plus = ExprNode(
            (BinTreeDirection.LEFT, BinTreeDirection.LEFT),
            MathTex("+", **NODE_CONFIG).move_to(LEFT * 2.75 + UP * 1),
        )
        left_right_times = ExprNode(
            (BinTreeDirection.LEFT, BinTreeDirection.RIGHT),
            MathTex("\\times", **NODE_CONFIG).move_to(RIGHT * 0.25 + UP * 1),
        )
        leaf1 = ExprNode(
            (BinTreeDirection.LEFT, BinTreeDirection.LEFT, BinTreeDirection.LEFT),
            MathTex("1", **NODE_CONFIG).move_to(LEFT * 3.5 + DOWN * 1),
        )
        leaf3 = ExprNode(
            (BinTreeDirection.LEFT, BinTreeDirection.LEFT, BinTreeDirection.RIGHT),
            MathTex("3", **NODE_CONFIG).move_to(LEFT * 2 + DOWN * 1),
        )
        leaf4 = ExprNode(
            (BinTreeDirection.LEFT, BinTreeDirection.RIGHT, BinTreeDirection.LEFT),
            MathTex("4", **NODE_CONFIG).move_to(LEFT * 0.5 + DOWN * 1),
        )
        leaf7 = ExprNode(
            (BinTreeDirection.LEFT, BinTreeDirection.RIGHT, BinTreeDirection.RIGHT),
            MathTex("7", **NODE_CONFIG).move_to(RIGHT * 1 + DOWN * 1),
        )
        leaf9 = ExprNode(
            (BinTreeDirection.RIGHT,),
            MathTex("9", **NODE_CONFIG).move_to(RIGHT * 2 + UP * 2),
        )
        ...
        def animate_reduction(
            ...
            rectangles_along_path = [
                ...
                for subtree in [
                    vgroup_containing_direct_children_along_with_node_at(
                        path_to_redex[:i]
                    )
                ]
            ]
            ...
            self.play(
                ...
                Succession(
                    last_rect.animate.scale(1.1).set_rate_func(
        ...
        expr = (
            MathTex("((1+3)+(4\\times7))+9", arg_separator="", **NODE_CONFIG)
            .scale(0.8)
            .to_edge(RIGHT, buff=1)
        )
...
class SearchRedexAndFoldExpressionTree_123456(Scene):
    ...
    def construct(self):
        ...
        l_plus = ExprNode(
            (BinTreeDirection.LEFT,),
            MathTex("+", **NODE_CONFIG).move_to(RIGHT * 1 + UP * 1.5),
        )
        ll_plus = ExprNode(
            (BinTreeDirection.LEFT, BinTreeDirection.LEFT),
            MathTex("+", **NODE_CONFIG).move_to(RIGHT * 0 + UP * 0),
        )
        lll_plus = ExprNode(
            (BinTreeDirection.LEFT, BinTreeDirection.LEFT, BinTreeDirection.LEFT),
            MathTex("+", **NODE_CONFIG).move_to(LEFT * 1 + DOWN * 1.5),
        )
        llll_plus = ExprNode(
            (
                BinTreeDirection.LEFT,
            ...
            MathTex("+", **NODE_CONFIG).move_to(LEFT * 2 + DOWN * 3),
        )
        leaf1 = ExprNode(
            (
                BinTreeDirection.LEFT,
            ...
            MathTex("1", **NODE_CONFIG).move_to(LEFT * 3 + DOWN * 4.5),
        )
        leaf2 = ExprNode(
            (
                BinTreeDirection.LEFT,
            ...
            MathTex("2", **NODE_CONFIG).move_to(LEFT * 1.5 + DOWN * 4.5),
        )
        leaf3 = ExprNode(
            (
                BinTreeDirection.LEFT,
            ...
            MathTex("3", **NODE_CONFIG).move_to(LEFT * 0.5 + DOWN * 3),
        )
        leaf4 = ExprNode(
            (BinTreeDirection.LEFT, BinTreeDirection.LEFT, BinTreeDirection.RIGHT),
            MathTex("4", **NODE_CONFIG).move_to(RIGHT * 0.5 + DOWN * 1.5),
        )
        leaf5 = ExprNode(
            (BinTreeDirection.LEFT, BinTreeDirection.RIGHT),
            MathTex("5", **NODE_CONFIG).move_to(RIGHT * 1.5 + UP * 0),
        )
        leaf6 = ExprNode(
            (BinTreeDirection.RIGHT,),
            MathTex("6", **NODE_CONFIG).move_to(RIGHT * 2.5 + UP * 1.5),
        )
        ...
        def animate_reduction(
            ...
            new_value_node = (
                MathTex(new_value, **NODE_CONFIG)
                .scale(0.8)
                .move_to(current_expr_tree[path_to_redex].get_center())
            )
            new_value_box = SurroundingRectangle(new_value_node, color=ORANGE, buff=0.1)
        ...
        expr = (
            MathTex(
                "((((1 + 2)", " + 3) + 4) + 5) + 6", arg_separator="", **NODE_CONFIG
            )
            .scale(0.8)
            .to_edge(RIGHT, buff=1)
        )
